...
jQuery(document).ready(function($){
    ...
    */
    ...
    jQuery('#addition-shortcut').click(function(){
        var template = jQuery('#pfw-shortcut-template').html();
        var sec = jQuery('.pwaforwp-repeat-sec').length
        template = template.replace(/%i%/g, sec-1)
        jQuery('.pwaforwp-repeat-wrap').append(template);
    })// #addition-shortcut click event closed
    
    jQuery(document).on('click','.pwaforwp-repeat-trash', function(e){
        e.preventDefault();
        var self = jQuery(this);
            self.parents('.pwaforwp-repeat-sec').remove();
    })// .pwaforwp-repeat-trash click event closed

    jQuery(document).on('click', ".pwaforwp-set-shortcuticon", function(e) {  // Application Icon upload
        var self = jQuery(this);
        e.preventDefault();
        var pwaforwpMediaUploader = wp.media({
            title: pwaforwp_obj.uploader_title,
        ...
        .on("select", function() {
            var attachment = pwaforwpMediaUploader.state().get("selection").first().toJSON();
            self.parent().find(".pwaforwp-icon").val(attachment.url);
        })
        .open();
    });
